...
## [CLOSED] Account Deletion credential verification
`app/src/main/java/com/ga/airdrop/feature/more2/AccountDeletionScreen.kt`, `AccountDeletionViewModel.kt`, and `More2Api.kt` — the entry screen looked close, but the Confirm flow used a stale backend endpoint instead of Swift's credential-verification rail.

**Detail:** Figma MCP for node `40007388:24881` currently returns a stale/misnamed reason-style screen with "Why do you want you want to delete your account?" and `Delete Account`. Swift `FigmaAccountDeletionViewController.swift` is the guide for the entry screen: email/password cards, pinned `Confirm`, local empty-field validation, and `verifyAccountCredentials(email:password:)`. Swift `AirdropAPI.verifyAccountCredentials` calls `APIEndpoints.login`, which is `/auth/login`. Android's `More2Api.verifyLogin` still posted to `login`, so Confirm could fail even when the Compose screen matched the Swift copy and layout.

**Fix:** Closed. Android now reuses the existing Account Deletion ViewModel/Repository path and points `verifyLogin` at `auth/login`; no duplicate route, no second credential flow, and the returned verification token is not persisted. `AccountDeletionScreen` gained test-only semantic tags for the root, fields, cards, bottom bar, and Confirm CTA. `More2ApiContractTest` locks the Retrofit annotation to `auth/login`; `AccountDeletionParityTest` verifies Swift entry copy over stale Figma reason copy, 20dp gutters, 50dp input cards, 52dp pinned Confirm, local empty-email validation, trimmed credential handoff, and bearer-token preservation during verification.

**Verification 2026-07-06:** `git diff --check`, `:app:compileProdDebugKotlin :app:compileProdDebugUnitTestKotlin :app:compileProdDebugAndroidTestKotlin`, focused JVM `More2ApiContractTest` passed 1/1, and focused Gradle `connectedProdDebugAndroidTest` for `AccountDeletionParityTest` passed 2/2 on `airdrop_test2(AVD) - 15`.
...
`Earn $2 USD Per Invite`, explanatory body copy, and one bottom `Invite` CTA
that routes to the separate Invite Friend screen. `ReferAFriendParityTest`
guards all three cards, the exact copy, the CTA callback, light/dark rendering,
...
passed 3/3. Prod connected Refer verification is not accepted yet because the
prod instrumentation run crashed before executing tests (`0 tests`).
