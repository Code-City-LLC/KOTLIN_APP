...
- PackagesFilterSheet now has Swift-precedence proof against Figma node
  ...
  Follow-up 2026-07-06: the sheet host itself was corrected from a fullscreen
  dialog to Swift's `.pageSheet` large-detent equivalent: bottom-aligned shell,
  dimmed backdrop, visible grabber, 24dp top radius, and navigation-inset safe
  area. `PackagesFilterSheetParityTest` now locks the non-fullscreen shell,
  bottom edge, grabber dimensions, and row/card geometry; current proof:
...
- Payments/Orders now have Swift-precedence proof against Figma Payments node
  ...
  more-square as a visual accessory without adding a fake action. Follow-up
...
- Description fallback is now Swift-first. Figma node `40002072:24025` supplies
  ...
  `/tmp/kotlin_ui_proof/product_description_fallback/figma/auction_product_details_40002072_24025.png`,
  `/tmp/kotlin_ui_proof/product_description_fallback/android/auction_description_fallback_swift_light.png`.
- Product Detail add-to-cart is now Swift-first. Figma node `40002072:24025`
  supplies the bottom `Add to Cart` surface, while Swift
  `FigmaAuctionProductDetailsViewController.onAddToCart` defines runtime
  behavior: add through `FigmaCartStore`, update badge state, show `Added to
  cart` / `Already in cart`, and let `View Cart` push My Cart. Android keeps the
  existing `CartStore` source of truth and now proves the Product Detail CTA
  adds exactly one package-backed line, preserves Swift's single-unit hosted
  checkout behavior even after the visual quantity stepper is increased,
  rejects duplicate lines, and routes both `View Cart` and the header Cart icon
  to `Routes.CART`.
- Shop root search/card/rail geometry is now screen-level proven against Figma
  node `40001846:53519` and Swift `FigmaShopViewController.swift`. The exact
  mismatch was `ShopSearchField` rendering 52.19dp high because it used a
  50dp minimum height; Swift and Figma require an exact 50pt/50px search card.
  Android now uses exact `height(50.dp)` and has stable root/search/card tags.
  `ShopRootScreenParityTest` proves in light and dark that the search field is
...
- Auction and Feature Products full-list bottom clearance now follows Swift.
  ...
  chrome. Swift `FigmaAuctionViewController.swift` and
  `FigmaFeatureProductsViewController.swift` set the grid section bottom to 30pt
  for row spacing, then separately set `contentInset.bottom = 124` and
  `verticalScrollIndicatorInsets.bottom = 124` because the collection view is
  constrained to the screen bottom. Android previously only had the 30dp
  section spacing. `ProductListScreen` now uses
  `ProductListBottomClearance = 124.dp` for both lists while preserving the
  existing card/grid/search/sort/cart rails. `ProductListBottomClearanceParityTest`
  proves Auction in light and Feature Products in dark after scrolling to the
  end of the actual screen; adjacent Shop regression proof passed 13/13 on
  `airdrop_test2(AVD) - 15`.
- Auction Product Detail full visual geometry is now guarded. Figma MCP was
  refreshed for Product Detail `40002072:24025`, and Swift
  `FigmaAuctionProductDetailsViewController.swift` remains the runtime guide for
  conflicts. Android already matched the Swift hero/card/dot/stats/title/model/
  price/stock/stepper/description/related/footer constants, so the production
  change is limited to non-visual test tags. New
  `AuctionProductDetailsFullVisualParityTest` locks those dimensions in light
...
- Cart hosted checkout now has direct Swift-precedence proof against Figma node
  ...
  Safari/hosted URL opening, and verify-then-clear cart timing: the cart is
  cleared only after a verified-paid return, not when the Custom Tab opens.
  Android preserves the existing Swift-first layout and now
  ...
  package-ID blocking, and verify-then-clear timing. Focused
...
- Cart Saved for Later now follows Swift `FigmaSavedForLaterStore` /
  `FigmaSavedForLaterViewController`: separate persisted saved list, newest
  first, `Saved (N)` pill hidden at zero, long-press `Save for Later` removes
  only the selected active cart line, `Move to Cart` restores it, `Remove`
  deletes only the saved copy, and logout/cache/deletion hygiene clears saved
  rows. Live Figma screenshot for Cart node `40008284:26547` was attempted on
  2026-07-06 but the Figma connector returned `UNAUTHORIZED` reauth required,
  so this closure is Swift-source-backed rather than fresh Figma-visual-backed.
...
- Header/footer opacity was rechecked after the latest Swift-precedence ruling.
  Figma Home node `40001464:28899` still shows translucent Home chrome, while
  Swift `FigmaTabHeader` and `FigmaBottomTabBar` layer blur underneath an opaque
  `gray200` overlay. Swift now wins this conflict. Android uses `AirdropChrome`
  as the single source of truth for opaque light/dark `gray200`; the Home chrome
...
- Shipment hub first-entry refresh ownership now follows Swift's
  `viewDidAppear` intent without duplicate first-entry API calls. Android keeps
  the ViewModel's initial hub load, removes the extra immediate resumed-state
  refresh from the lifecycle helper, and coalesces overlapping refresh jobs.
  `ShipmentsHubTapRailsParityTest` verifies one first-entry call and a later
  `ON_RESUME` reload.
...
- Package detail surfaces, timeline rendering, charges footer, Packages filter
  shell/rows, Payments refresh/invoice/filter action sheet, and the covered
  detail-screen parity issues above now have focused Swift-first proof. Broader
  authenticated full-flow parity remains open.
...
| Cart / hosted checkout / Saved for Later | `feature/cart/CartScreen.kt`, `CartViewModel.kt`, `CartStore.kt`, shared checkout helper | `FigmaCartViewController.swift`, `FigmaSavedForLaterStore.swift`, `FigmaSavedForLaterViewController.swift` | `40008284:26547` | `/payments/create-checkout`, `/exchange-rates`, `/user` billing profile, local saved-list prefs | yes | inherited | yes | MagentaCastle/Codex | Cart hosted checkout has direct Swift-precedence functional proof: USD payload, `is_auction=true`, sorted package IDs, hosted URL open, verify-then-clear (cart stays until verified-paid return), missing package-ID block, and Swift unauthenticated alert copy; Saved for Later has Swift-source proof and connected Android proof for save/move/remove/separate-count behavior; live Figma screenshot was reauth-blocked on 2026-07-06 |
