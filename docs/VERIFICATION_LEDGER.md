...
### P0 — 🔐 SECURITY: remove legacy live-chat widget from active Kotlin chat path (KEMAR #14792)
- **Status:** Kotlin active source no longer embeds the legacy third-party widget. `LiveAgentChatScreen.kt` is native Compose and routes through the same AutoPilot/Nirvana app-channel shape used by Swift: Airdrop Laravel `POST /api/v1/autopilot/identity`, then direct app-channel session/messages calls with per-user publishable/user/hash headers.
- **Important security boundary:** direct AutoPilot calls must not reuse the authenticated Airdrop bearer client. Kotlin uses a separate OkHttp client for app-channel calls and sends only `x-autopilot-publishable-key`, `x-autopilot-user-id`, `x-autopilot-identity-hash`, and `x-autopilot-client-app`.
- **Kotlin footprint:** [LiveAgentChatScreen.kt](app/src/main/java/com/ga/airdrop/feature/contacts/LiveAgentChatScreen.kt), [LiveAgentChatViewModel.kt](app/src/main/java/com/ga/airdrop/feature/contacts/LiveAgentChatViewModel.kt), [LiveAgentChatRepository.kt](app/src/main/java/com/ga/airdrop/feature/contacts/LiveAgentChatRepository.kt), [Routes.kt](app/src/main/java/com/ga/airdrop/core/navigation/Routes.kt), [AppRoot.kt](app/src/main/java/com/ga/airdrop/core/navigation/AppRoot.kt).
- **DO NOT remove:** call/email/WhatsApp/location Help-tab actions, and `FigmaIcon_Chat`/`FigmaIcon.chat` notification-category icons — those are NOT the live-chat provider.
- **Verification required before close:** source grep for old widget URLs/keys, Kotlin compile, live-chat repository decode tests, Contacts route test, and rendered live-chat shell test.
