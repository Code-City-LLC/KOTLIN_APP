...
### P7 — 🔴 FALSE SWIFT-SOURCE CITATIONS + checkout-timing divergence (BlueDeer, both-repo-verified 2026-07-06, #15843; awaiting Kemar)
Two newly-landed features are net-new (NO Swift source) yet cite **non-existent Swift symbols** as "Swift-source proof"; owner per docs = MagentaCastle/Codex. I inspected BOTH repos personally (not relaying the Workflow).
- **`5998dd2` Cart Saved-for-Later:** cites `Swift FigmaSavedForLaterStore` ([CartStore.kt](app/src/main/java/com/ga/airdrop/feature/cart/CartStore.kt) L683) + `FigmaSavedForLaterViewController` ([CartScreen.kt](app/src/main/java/com/ga/airdrop/feature/cart/CartScreen.kt) L520) + `docs/FULL_APP_SWIFT_FIGMA_AUDIT.md` table (both `.swift` files, "Swift-source proof"). **VERIFIED FALSE:** `git -C SWIFT_APP ls-files | grep -i savedforlater` = 0; `grep saveForLater/savedForLater *.swift` = 0. Swift `FigmaCartStore` has only add/remove/toggle/contains/removeAll. Invented too: "Basket (N Items)" header + orange "Saved (N)" pill vs Swift plain `sectionHeader("Packages")`. Docs admit Figma was `UNAUTHORIZED` at closure → feature is verified against NEITHER Swift NOR Figma.
- **`5998dd2` also changed CHECKOUT-CLEAR timing (HIGH, restricted):** Swift clears the cart *when the checkout URL opens* — `FigmaCartViewController.swift:962-965` (`removeAll()` inside `present(safari…){ }`). Commit rewrote Kotlin to *keep-until-verified-paid* (CartStore doc L17-18; test `makePayment…KeepsCartUntilVerifiedReturn`; docs "verify-then-clear") and labels it "direct Swift-precedence proof" — the **opposite** of Swift. New UX may be preferable but it DIVERGES + is mislabeled; Kemar to rule keep-vs-revert.
- **`4e60dd0` Report-Damage:** cites `Swift FigmaPackageDetailsViewController.updateReportDamageCTA` ([PackageDetailsViewModel.kt](app/src/main/java/com/ga/airdrop/feature/shipments/PackageDetailsViewModel.kt) L567). **VERIFIED FALSE:** Swift package-details VC has only the "Add to Cart" CTA (`FigmaPackageDetailsViewController.swift:869`, status ≥ 7); `grep reportDamage|damage-reports *.swift` = 0; `AirdropAPI.swift` has no damage endpoint. Invents `POST /packages/{id}/damage-reports` + `photos[]` (5-photo/10MB) with no Swift/backend counterpart.
- **NOT fake (excluded — corrects the Workflow over-reach):** `canDeleteInvoices(for:)` is the REAL pre-existing invoice-delete gating (KEMAR #15084 status ≥ 7 || name). Leave it.
- **Ask (Kemar/@MagentaCastle):** (a) sanctioned Figma/product features or revert to Swift? (b) if kept, correct the false Swift citations in code + `docs/FULL_APP_SWIFT_FIGMA_AUDIT.md` + `docs/PARITY_BACKLOG.md`; (c) rule on checkout-clear timing. I did NOT edit others' code; I'll draft the citation fix or re-verify on ruling.
