...
data class PendingHostedCheckout(
    ...
    // which self-heals installs that are already stuck.
...
object CheckoutFlowStore {
    ...
    /**
     * Deadlock fix: abandoned checkouts (closed Custom Tab, crashed return, or
     * the fragment-rejection bug in [validatedHostedCheckoutUrl]) previously
     * blocked start() FOREVER because nothing cleared the pending records
     * outside the payment-return deeplink — and the block sits BEFORE the
     * currency split, so it killed the NCB/JMD rail too. Expire them on
     * Stripe's own lifetimes instead. Legacy rows (createdAtMs=0) expire
