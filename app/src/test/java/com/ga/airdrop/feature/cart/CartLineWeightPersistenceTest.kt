...
class CartLineWeightPersistenceTest {
    ...
    @Test
    fun `mixed legacy and weighted lines sum only the known weights`() {
        val lines = listOf(
            CartStore.CartLine(id = 1, title = "legacy", qty = 1, priceUsd = 1.0),
            CartStore.CartLine(id = 2, title = "weighted", qty = 1, priceUsd = 1.0, weightKg = 2.5),
        )
        val total = lines.mapNotNull { it.weightKg }.sum().takeIf { it > 0 }
        assertEquals(2.5, total!!, 1e-9)
    }
}
